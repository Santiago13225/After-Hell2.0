...
if timer >= spawnTime && !place_meeting(x, y, oPurpleEyedZombie)
{
	//create an enemy
	//var _inst = instance_create_depth(x, y, depth - 1, oZombie);
	var _inst = instance_create_depth(x, y, depth - 1, oPurpleEyedZombie);
	with(_inst)
	{
...
if timer >= spawnTime && !place_meeting(x, y, oZombie)
{
	// Choose a random zombie type from the array
	var chosenZombieType = choose(zombieTypes);
