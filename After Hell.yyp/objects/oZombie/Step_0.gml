...
/*
	...
	if(screen_pause()){//if screen is paused
		xspd = 0;//set dumb-chase movement speed to 0
		yspd = 0;
	}else{//else...
		if(!usingPathfinding){//if not using pathfinding behavior
			xspd = lengthdir_x(spd, dir);//set dumb-chase speed
			yspd = lengthdir_y(spd, dir);
		}/*else{
			xspd = 0;
			yspd = 0;
