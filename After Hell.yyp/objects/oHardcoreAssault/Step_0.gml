///oHardcoreAssault Step Event
/*This object represents the hardcore assault object.*/
//This event is responsible for handling hardcore assault object behavior.
...
for(var i = 0; i < _size; i++){
	...
	if(global.PlayerWeapons[i] == global.WeaponList.hassault){
		_weaponPos = i;
		_playerHas = true;
		i += 99;
		//exit;
...
if(place_meeting(x, y, oPlayer)){
    ...
    if(item_add(item) == true){
        ...
        //array_push(global.PlayerWeapons, weapon);
		if(!_playerHas){
			array_push(global.PlayerWeapons,  global.WeaponList.hassault);
			//array_push(global.PlayerAmmo, 0);
			array_push(global.PlayerMag, global.WeaponList.hassault.magSize);//start with full mag
