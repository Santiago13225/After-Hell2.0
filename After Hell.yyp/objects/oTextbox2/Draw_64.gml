...
textbox_x = _wx/2 - (textbox_width/2 * 3) - 42;//X-position of the textbox.
//textbox_x = (camera_get_view_x(view_camera[0])/2) + (camera_get_view_width(view_camera[0])/2) -textbox_width/4;
textbox_y = _wy - (textbox_height * 3);//Y-position of the textbox.
...
if(setup == false){
	...
	for(var p = 0; p < page_number; p++){//For each page...
...
//Flip Through Pages
if(accept_key){//If the accept key has been pressed.
	if(draw_char == text_length[page]){//If the typing is done.
...
draw_sprite_ext(txtb_spr, txtb_img, _txtb_x, _txtb_y, textbox_width/txtb_spr_w * 3, textbox_height/txtb_spr_h * 3, 0, c_white, 1);//Back of the textbox.


//Options
if(draw_char == text_length[page] && page == page_number - 1){//If we are at the point where the last text has been printed and the last page has been printed.
	//Option Selection
	...
	for(var op = 0; op < option_number; op++){//For each option...
		//Draw the option box.
		...
		//Arrow
		if(option_pos == op){//Wherever the position is...
...
draw_text_ext_transformed(_txtb_x + border, _txtb_y + border - 6, _drawtext, line_sep, line_width, 3, 3, 0);//Draw the text.