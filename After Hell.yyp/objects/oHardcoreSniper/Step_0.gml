///oHardcoreSniper Step Event
/*This object represents the hardcore sniper.*/
//This event is responsible for handling hardcore sniper object behavior.
...
for(var i = 0; i < _size; i++){
	...
	if(global.PlayerWeapons[i] == global.WeaponList.hsniper){
		_weaponPos = i;
		_playerHas = true;
		i += 99;
		//exit;
...
if(place_meeting(x, y, oPlayer)){
    ...
    if(item_add(item) == true){
        ...
        //array_push(global.PlayerWeapons, weapon);
		if(!_playerHas){
			array_push(global.PlayerWeapons,  global.WeaponList.hsniper);
			//array_push(global.PlayerAmmo, 0);
			array_push(global.PlayerMag, global.WeaponList.hsniper.magSize);//start with full mag
